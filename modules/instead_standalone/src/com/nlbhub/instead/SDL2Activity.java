 ...
 * SDL Activity
 */
public class SDL2Activity extends SDL2ActivityBase {
	...
	private static SDL2Activity Ctx;
	...
	*/
	...
	public static SDL2Activity getCtx() {
		return Ctx;
	}
	...
	protected SDL2SurfaceBase initSurface() {
		return new SDL2Surface(getApplication());
	}
 ...
 */
class SDL2Main implements Runnable {
	private String dataDir;

	public SDL2Main(String dataDir) {
		this.dataDir = dataDir;
	}

	public void run() {
		final SDL2Activity ctx = SDL2Activity.getCtx();
		final ExpansionMounter expansionMounter = SDL2Activity.getExpansionMounterMain();
		...
		Settings settings = SDL2Activity.getSettings();
...
class SDL2Surface extends SDL2SurfaceBase {
	private Context context;

	public SDL2Surface(Context context) {
		super(context);
	...
	public boolean onKey(View v, int keyCode, KeyEvent event) {
		SDL2Activity.refreshOff();
		int key = keyCode;

		if (SDL2Activity.getSettings().getOvVol()) {
			switch (keyCode) {
		...
		switch (actionMasked) {
			case MotionEvent.ACTION_MOVE:
				for (i = 0; i < pointerCount; i++) {
					...
					SDL2Activity.onNativeTouch(touchDevId, pointerFingerId, actionMasked, x, y, p);
				}
				break;
	...
	protected Thread initThread() {
		final Thread thread = new Thread(new SDL2Main(getDataDir()), "SDLThread");
		...
		new Thread(new Runnable(){
			...
			public void run(){
				try {
					...
					if (! SDL2Activity.mExitCalledFromJava) {
						SDL2Activity.handleNativeExit();
					}
				}
			}
		}).start();
